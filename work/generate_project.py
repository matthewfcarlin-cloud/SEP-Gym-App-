...
obj('product', 'isa = PBXFileReference; explicitFileType = wrapper.application; includeInIndex = 0; path = Borrow.app; sourceTree = BUILT_PRODUCTS_DIR;')
...
obj('sourceGroup', 'isa = PBXGroup; children = ('+','.join(uid(f.name) for f in files)+'); path = Borrow; sourceTree = "<group>";')
...
for name in ('Debug','Release'):
    ...
    obj('target'+name, f'''isa = XCBuildConfiguration; name = {name}; buildSettings = {{
      PRODUCT_NAME = "$(TARGET_NAME)"; PRODUCT_BUNDLE_IDENTIFIER = com.demo.borrow;
      GENERATE_INFOPLIST_FILE = YES; INFOPLIST_KEY_CFBundleDisplayName = Borrow;
      INFOPLIST_KEY_NSCameraUsageDescription = "Scan an equipment QR tag to find and rent that item.";
...
obj('target', f'isa = PBXNativeTarget; buildConfigurationList = {uid("targetConfigList")}; buildPhases = ({uid("sources")},{uid("frameworks")},{uid("resources")}); buildRules = (); dependencies = (); name = Borrow; productName = Borrow; productReference = {uid("product")}; productType = "com.apple.product-type.application";')
...
(root/'Borrow.xcodeproj/project.pbxproj').write_text('// !$*UTF8*$!\n{ archiveVersion = 1; classes = {}; objectVersion = 56; objects = {\n'+'\n'.join(objects)+'\n}; rootObject = '+uid('project')+'; }\n')
ref = f'<BuildableReference BuildableIdentifier="primary" BlueprintIdentifier="{uid("target")}" BuildableName="Borrow.app" BlueprintName="Borrow" ReferencedContainer="container:Borrow.xcodeproj"/>'
(root/'Borrow.xcodeproj/xcshareddata/xcschemes/Borrow.xcscheme').write_text(f'''<?xml version="1.0" encoding="UTF-8"?>
